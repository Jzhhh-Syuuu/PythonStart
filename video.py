...
class bilibili():
    ...
    def download_video(self,video_list):
        for item in video_list:
            ...
            path = r'D:\PythonDownload\bilibili\{dir_title}'.format(dir_title=title)
    ...
    def CombineVideoAudio(self, video_list):
        ...
        for item in video_list:
            ...
            log_filepath = r'D:\PythonDownload\bilibili\{dir_title}\{log_filename}'.format(dir_title=title,log_filename=log_filename)
            video_filename = name + '.mp4'
            video_filepath = r'D:\PythonDownload\bilibili\{dir_title}\{video_filename}'.format(dir_title=title,video_filename=video_filename) # 设置图片name，注：必须加上扩展名
            audio_filename = name + '.mp3'
            audio_filepath = r'D:\PythonDownload\bilibili\{dir_title}\{audio_filename}'.format(dir_title=title,audio_filename=audio_filename)# 设置图片name，注：必须加上扩展名
            out_filename = name + '-change.mp4'
            out_filepath = r'D:\PythonDownload\bilibili\{dir_title}\{out_filename}'.format(dir_title=title,out_filename=out_filename)
...
if __name__ == '__main__':
    ...
    thirdpath = r'D:\PythonDownload\bilibili'
    os.chdir(firstpath)
