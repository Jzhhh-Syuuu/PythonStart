...
class bilibili():
    ...
    def parseHtml(self, res_list):
        ...
        for item in res_list:
            ...
            html = item['response']
            doc = etree.HTML(html)
            video_dict['title'] = doc.xpath(
                "//div[@id='viewbox_report']/h1/span/text()")[0]
            # 调试输出结果
            # print(doc)
    ...
    def download_video(self, video_list):
        for item in video_list:
            ...
            r = requests.get(url=video_url, stream=True,
                             headers=self.downloadVideoHeaders)
            ...
            print('[Start download]:{filename},[File size]:{size:.2f} MB'.format(
                filename=filename, size=length / 1024 / 1024))  # 开始下载，显示下载文件大小
            path = r'D:\PythonDownload\bilibili\{dir_title}'.format(
                dir_title=title)
    ...
    def CombineVideoAudio(self, video_list):
        ...
        for item in video_list:
            ...
            video_filepath = r'D:\PythonDownload\bilibili\{dir_title}\{video_filename}'.format(
                dir_title=title, video_filename=video_filename)  # 设置图片name，注：必须加上扩展名
            audio_filename = name + '.mp3'
            audio_filepath = r'D:\PythonDownload\bilibili\{dir_title}\{audio_filename}'.format(
                dir_title=title, audio_filename=audio_filename)  # 设置图片name，注：必须加上扩展名
